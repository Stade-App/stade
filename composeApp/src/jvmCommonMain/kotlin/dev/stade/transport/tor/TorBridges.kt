...
fun parseBridgeConfig(raw: String): TorBridgeConfig {
    ...
    raw.lineSequence().forEach { rawLine ->
        ...
        when (key) {
            "bridgesEnabled" -> enabled = value.equals("true", ignoreCase = true)
            "bridgesUseBuiltIn" -> useBuiltIn = value.equals("true", ignoreCase = true)
            "bridge" -> if (value.isNotEmpty()) custom.add(value)
        }
    }
    return TorBridgeConfig(enabled, useBuiltIn, custom)
}
